...
selftriggerch = 3 ## channel number in oscilloscope
selftriggerFCCH = 3 ## channel number in FC.

########### root output file 
f = ROOT.TFile("data/SelfTrigger_FCCH{}_{}.root".format(selftriggerFCCH, dt_now.strftime('%Y%m%d-%H%M')) 
               , "RECREATE")
tree = ROOT.TTree("PMT", "Integrated palse")
...
channel_number = 1
trigger_channel = selftriggerch ## channel number for trigger 
...
for ch in range(4):
    ds.SetVerticalDiv("C{}".format(ch +1),vdiv) ## 50 mV/div
    ds.SetOffset("C{}".format(ch+1), voffset) ## set offset for CH1 to -150 mv.
...
while i < attempts:
    ...
    CH3[0] = integral[2]
    tree.Fill()
    print('Event {} {:.3f} FC CH0 {:.6f} CH1 {:.6f} CH3 {:.6f}'.format(i, 
                                                                       time.time(),
                                                                       CH0[0],
                                                                       CH1[0],
                                                                       CH3[0]), flush=False)
